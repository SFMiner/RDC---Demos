...
func restore_scene_from_saved_state():
	...
	if debug: DebugManager.print_debug_auto(self, "scene_visited = " + str(GameState.scene_visited(scene_instance.name)))
	if GameState.scene_visited(scene_instance.name):
		GameState.clear_pickups()
		if debug: DebugManager.print_debug_auto(self, str(GameState.get_pickups()))
		...
		for pickup in pickups_to_put_down:
			...
			if debug: DebugManager.print_debug_auto(self, "Pickup " + restored_pickup.item_id + " restored at " + str(restored_pickup.position))
...
func create_pickup_from_data(pickup_data: Dictionary):
	...
	if not pickup_scene:
		if debug: DebugManager.print_debug_auto(self, "ERROR: Could not load pickup_item.tscn")
		return null
...
func register_pickup(pickup_node):
	if not pickup_node or not pickup_node.has_method("get_pickup_save_data"):
		if debug: DebugManager.print_debug_auto(self, "Invalid pickup node")
		return
...
func drop_item_in_world(pickup_data : Dictionary) -> void:
	...
	if debug: DebugManager.print_debug_auto(self, "Dropped item " + pickup_data.item_id + " at " + str(pickup_data.position) + " with ID: " + pickup_id)
...
func load_save_data(data: Dictionary) -> bool:
	...
	if debug: DebugManager.print_debug_auto(self, "Pickup system restoration complete")
	return true
...
func reset():
	...
	active_pickups.clear()
	if debug: DebugManager.print_debug_auto(self, "Pickup system reset")
